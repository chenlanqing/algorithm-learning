...
//        arr[min] = temp;
	...
	public static void printToTree(int[] arr) {
		printToTree(arr, arr.length);
	}

	public static void printToTree(int[] arr, int len) {
		final Integer[] array = Arrays.stream(arr).boxed().limit(len).toArray(Integer[]::new);
		final TreeNode<Integer> root = TreeUtils.buildTree(array, new TreeNode<>(arr[0]), 0);
